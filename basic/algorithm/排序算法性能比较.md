...
这种算法稍复杂一些，首先你需要了解堆结构，它是一颗近似完全二叉树的数据结构，并且需要将它调整大顶堆或小顶堆，也就是说父节点总是大于（小于）或等于任何一个子节点，
堆化后，将堆顶元素与堆最后一个元素进行交换，堆的最后一个元素将不再参与下一轮的堆化，交换后继续堆化，直到堆的大小等于`1`，整个堆排序完成

所以堆排序的重点是如何调整最大（小）堆，如果用数组表示堆的话，父节点为`i`的节点，其子节点分别为`2*i+1`、`2*i+2`，从`n/2`的父节点开始，
...
|算法名称|一千数据量|一万数据量|十万数据量|
|--------|--------|--------|--------|
|暴力排序|52ms|239ms|37883ms|
|冒泡排序|11ms|177ms|18665ms|
|插入排序|5ms|30ms|1430ms|
|快速排序|1ms|4ms|27ms|
|选择排序|11ms|74ms|5080ms|
|希尔排序|1ms|9ms|22ms|
|归并排序|4ms|9ms|41ms|
|堆排序|3ms|8ms|16ms|
