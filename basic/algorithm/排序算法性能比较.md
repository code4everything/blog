...
在学习了这些常用排序算法之后，接下来我们来看看这些排序算法对十万数据量的数组进行排序到底需要多久呢，本来我想测一个百万数据量的数组的，
但是等的太TMD的久了，还是放弃了，测一个十万长度的数组就行啦

|算法名称|时间复杂度|空间复杂度|运行时长|
|--------|--------|--------|--------|
|暴力排序|`O(n^2)`|`O(1)`||
|冒泡排序|`O(n^2)`|`O(1)`||
|插入排序|`O(n^2)`|`O(1)`||
|快速排序|<code>O(nlog<sub>2</sub>n)</code>|<code>O(nlog<sub>2</sub>n)</code>||
|选择排序|`O(n^2)`|`O(1)`||
|希尔排序|`O(n^1.3)`|`O(1)`||
|归并排序|<code>O(nlog<sub>2</sub>n)</code>|`O(n)`||
|堆排序|<code>O(nlog<sub>2</sub>n)</code>|`O(1)`||

性能排名：

``` text
堆排序 > 快速排序 > 希尔排序 > 归并排序 > 插入排序 > 选择排序 > 冒泡排序 > 暴力排序
```

大多数情况下，排名还是比较稳定的，偶尔`快速排序`和`希尔排序`以及`归并排序`会出现排名变动的情况
