...
归并排序采用了经典的分治策略，将大问题拆分成小问题逐个求解，比如这里的归并排序，将一个数组拆分两个序列，在分别将这两个序列再拆分成两个序列，
直到序列长度为`1`，然后依次向上对最这两个序列合并排序，这样每次我们合并的都是两个有序的序列，时间复杂度<code>O(nlog<sub>2</sub>n)</code>，
空间复杂度`O(n)`
